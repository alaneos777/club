...
vector<int> multiEvaluate(const poly & P){
	int n = points.size();
...
//finds a polynomial P(x) such that P(i) = y[i]
...
int main(){
	...
	poly F = interpolate(f);
	vector<int> y(m);
	for(int i = 0; i < m; ++i){
		y[i] = (c+i)%p;
	}
	vector<int> ans = multiEvaluate(F, y);
	for(int ai : ans){
		cout << ai << " ";
	}
	cout << "\n";
